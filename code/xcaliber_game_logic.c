...
/* static void */
/* demo_bunch_of_shapes (xc_context *ctx, stack_arena *arena) */
/* { */
/*   /\* draws a bunch of shit *\/ */
/*   xcr_set_background_colour (ctx->renderer_ctx, XC_BLACK); */
/*   int32_t x = 0; */
/*   int32_t y = 0; */
...
/* static void */
/* demo_rotating_triangle (xc_context *ctx, stack_arena *arena) */
/* { */
...
{
  /* demo_bunch_of_shapes (ctx, arena); */
  /* demo_rotating_triangle (ctx, arena); */

  xcr_set_background_colour (ctx->renderer_ctx, xc_preset_colour (XC_BLACK));

  xc_draw_command line = make_draw_command_line2d ((xc_line2d) { .start = { .x = 0, .y = 0 }, .end = { .x = 800, .y = 600 } }, xc_preset_colour (XC_WHITE));

  xcr_draw (ctx->renderer_ctx, arena, &line);
}
